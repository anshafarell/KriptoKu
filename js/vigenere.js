function cipherText(){
    ...
    var lengthStr = parseInt(str.length);
    var lengthKey = parseInt(keyless.length);
    var ciphertext = ""; 

    var conkey = 0,tempkey, tempstr, x;

    if(lengthKey <= lengthStr){
...
function decrypt(){
    ...
    var lengthCipher = parseInt(ciphertext.length);
    var lengthKey = parseInt(keyLess.length);

    if(lengthKey <= lengthCipher){
